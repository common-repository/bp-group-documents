...
class BP_Group_Documents {
	 ...
	 * icon()
	 *
	 * Gets the file extension from the documents, and displays the applicable icon 
	 */
	public function icon() {
		$icon_url = $this->get_icon();
		if ( false !== $icon_url ) {
			echo '<img class="bp-group-documents-icon" src="' . $icon_url . '" alt="' . $this->file_extension . ' icon" />';
		}
	}
	...
	/**
	 * Total number of documents in the given group and given category (if one given)
	 * @global type $wpdb
	 * @param int $group_id
	 * @param int $category
	 * @return type
	...
	public static function get_list_for_usergroups_widget( $num, $featured = 0 ) {
		...
		if ( bp_has_groups( 'user_id=' . get_current_user_id() ) ) {
			...
			$group_array = $user_groups['groups'];
